...
# -------------------------------------------
async def process_sanofi_html(html_content):
    """
    ...
    try:
        ...
        response = requests.get("https://www.sanofi.com/en/our-science/our-pipeline")
        ...
        for proj in deduped_projects:
            try:
                ...
                if country_list:
                    # e.g. "EU, JP, CN"
                    country_mld.add_translation("en", ", ".join(country_list))
                else:
                    country_mld.add_translation("en", "")
                ...
                name_mld.add_translation("en", name_str)

                indication_mld = MultilingualData()
                indication_mld.add_translation("en", cleaned_indication_str)

                description_mld = MultilingualData()
                description_mld.add_translation("en", description_str)

                therapeutic_area_mld = MultilingualData()
                therapeutic_area_mld.add_translation("en", therapeutic_area)

                phase_mld = MultilingualData()
                phase_mld.add_translation("en", phase_str)

                partner_mld = MultilingualData()
                partner_mld.add_translation("en", collab_str)

                notes_mld = MultilingualData()
                notes_mld.add_translation("en", notes_str)

                filing_mld = MultilingualData()
                filing_mld.add_translation("en", submission_str)

                date_updated_mld = MultilingualData()
                date_updated_mld.add_translation("en", date_last_updated)
